...
(function () {
    ...
    _proto.initImg = function () {
        ...
        var L = this.options.sliderL; // 滑块实际边长
        console.log(L)
        ...
        var img = new Image();
        var imgBolock = new Image();
        img.crossOrigin = "Anonymous";
        imgBolock.crossOrigin = "Anonymous";
        ...
        imgBolock.onload = function () {
            drawImg(that.blockCtx, 'fill');
            that.blockCtx.drawImage(imgBolock, 0, 0, 51, 170);
            var ImageData = that.blockCtx.getImageData(0, 0, 51, 170);
            that.blockCtx.putImageData(ImageData, 0, 0);
            that.text.textContent = that.text.getAttribute('data-text');
        };
        imgBolock.setSrc = function (small) {
            imgBolock.src = 'data:image/png;base64,' + small;
        };
        imgBolock.setSrc();
        img.setSrc = function (big) {
            img.src = 'data:image/png;base64,' + big;
        };
        img.setSrc();
        this.text.setAttribute('data-text', this.options.barText);
        this.text.textContent = this.options.barText;
        this.imgBolock = imgBolock;
        this.img = img;
    };

    _proto.clean = function () {
        this.canvasCtx.clearRect(0, 0, this.options.width, this.options.height);
        this.blockCtx.clearRect(0, 0, 51, 170);
    ...
    _proto.bindEvents = function () {
        ...
        var handleDragMove = function (e) {
            ...
            var blockLeft = (that.options.width - 40) / (that.options.width - 40) * moveX;
            that.block.style.left = blockLeft + 'px';
            that.sliderContainer.classList.add('sliderContainer_active');
            that.sliderMask.style.width = (moveX + 4) + 'px';
            var point={X:moveX,Y:moveY}
            trail.push(point);
        };
    ...
    _proto.reset = function (big, small) {
        if (big && small) {
